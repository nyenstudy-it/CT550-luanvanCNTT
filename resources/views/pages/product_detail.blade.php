                            ...
                            @foreach ($product->variants as $variant)
                                @forelse ($variant->images as $img)
                                    <img src="{{ asset('storage/' . $img->image_path) }}"
                                        ...
                                        class="variant-image"
                                        alt="{{ $product->name }} - {{ $variant->color ?? $variant->volume ?? $variant->weight ?? $variant->size ?? $variant->sku }}"
                                        onerror="this.src='{{ asset('frontend/images/product/product-1.jpg') }}';">
                                @empty
                                ...
                                @endforelse
                            @endforeach
                            {{-- Product-level images (without variant) --}}
                            @foreach(($product->images ?? collect())->whereNull('product_variant_id') as $img)
                                <img src="{{ asset('storage/' . $img->image_path) }}"
                                    data-imgbigurl="{{ asset('storage/' . $img->image_path) }}"
                                    data-variant-id=""
                                    data-is-primary="{{ $img->is_primary ? 1 : 0 }}"
                    ...
                    <div class="product__details__text">
                        ...
                        <div class="product__details__price" id="priceWrap">
                            @if($productPricing['has_discount'])
                                <span id="price" class="text-danger">{{ number_format((int) max(0, $productPricing['final_price'] ?? 0), 0) }} đ</span>
                                <small class="text-muted d-block"><del id="basePrice">{{ number_format((int) max(0, $productPricing['base_price'] ?? 0), 0) }} đ</del> <span id="discountBadge">{{ $productPricing['discount_label'] }}</span></small>
                            @else
                                <span id="price">{{ number_format((int) max(0, $productPricing['base_price'] ?? 0), 0) }} đ</span>
                            @endif
                        </div>
                            ...
                            <div class="d-flex flex-wrap gap-2 mt-2">
                                @foreach ($product->variants as $variant)
                                    ...
                                    <button type="button" class="variant-btn btn btn-outline-secondary btn-sm"
                                        data-id="{{ $variant->id }}"
                                        data-price="{{ (float) ($variantPriceData['base_price'] ?? 0) }}"
                                        data-final-price="{{ (float) ($variantPriceData['final_price'] ?? 0) }}"
                                        data-has-discount="{{ $variantPriceData['has_discount'] ? 1 : 0 }}"
                                        data-discount-label="{{ $variantPriceData['discount_label'] ?? '' }}"
                                        data-stock="{{ max(0, (int) ($variant->inventory?->quantity ?? 0)) }}"
                                        data-manufacture-date="{{ $variant->display_manufacture_date ? \Carbon\Carbon::parse($variant->display_manufacture_date)->toDateString() : '' }}"
                                        data-expired-at="{{ $variant->display_expired_at ? \Carbon\Carbon::parse($variant->display_expired_at)->toDateString() : '' }}"
                                        data-image="{{ $variantMainImage ? asset('storage/' . $variantMainImage) : asset('frontend/images/product/product-1.jpg') }}">
                        ...
                        <ul>
                            ...
                            <li><b>Tình trạng</b>
                                <span id="stockStatus" class="fw-semibold">{{ max(0, (int) ($product->inventory_total ?? 0)) > 0 ? 'Còn hàng' : 'Hết hàng' }}</span>
                            </li>
                            <li><b>Chia sẻ</b>
                                <div class="share">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (wishlistBtn) {
            wishlistBtn.addEventListener('click', function (e) {
                ...
                const wishlistUrl = @json(route('wishlist.toggle', $product->id));
